...
resource "aws_instance" "test-instance" {
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ${aws_instance.test-instance.public_ip}, --private-key ${var.private_key} docker.yml -e 'ansible_python_interpreter=/usr/bin/python3'" 
  }
